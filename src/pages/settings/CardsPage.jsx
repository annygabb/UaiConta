...
export default function CardsPage() {
  ...
  async function reload() {
    ...
    try {
      ...
      setError(err?.message || 'Não foi possível carregar seus cartões.')
    } finally {
      setLoading(false)
    }
  }
  ...
  async function save(event) {
    ...
    if ((closingDay && (closingDay < 1 || closingDay > 31)) || (dueDay && (dueDay < 1 || dueDay > 31))) {
      setError('Fechamento e vencimento precisam estar entre os dias 1 e 31.')
      return
    }
  ...
  return (
    <div className="page-stack">
      <div className="page-intro crud-intro">
        <div><span className="eyebrow">Pagamentos</span><h1>Cartões e Pix</h1><p>Cadastre cada cartão e acompanhe separadamente quanto saiu no crédito, no débito e via Pix.</p></div>
        <button className="primary-btn" onClick={() => setOpen(true)}><IconPlus size={17}/> Cadastrar cartão</button>
      </div>
      ...
      {loading ? <div className="panel crud-loading">Carregando...</div> : (
        <div className="payment-folder-grid cards-registry-grid">
          <PaymentFolder
            title="Pix"
            subtitle="Gasto realizado neste mês"
            amount={summary.pix / 100}
            icon={IconQrcode}
            accent="#8F5CE0"
            breakdown={[{ label: 'Total via Pix', value: summary.pix / 100 }]}
          />
          {summary.cardData.map(({ card, credit, debit, total }) => (
            <div className="registered-card-folder" key={card.id}>
              <PaymentFolder
                title={card.name}
      ...
      {open && <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}>
        <section className="crud-dialog" role="dialog" aria-modal="true" aria-label="Cadastrar cartão">
          ...
          <form className="crud-form" onSubmit={save}>
            ...
            <label><span>Dia de fechamento</span><input type="number" min="1" max="31" value={form.closingDay} onChange={(e)=>setForm((p)=>({...p,closingDay:e.target.value}))} /></label>
            <label><span>Dia de vencimento</span><input type="number" min="1" max="31" value={form.dueDay} onChange={(e)=>setForm((p)=>({...p,dueDay:e.target.value}))} /></label>
